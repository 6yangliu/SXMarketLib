...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _titleArray = @[ALS(@"开"), ALS(@"高"), ALS(@"低"), ALS(@"收"), ALS(@"涨跌额"), ALS(@"涨跌幅"), ALS(@"成交量")];
    }
...
//            cell.detailTextLabel.text = [NSString stringWithFormat:@"%0.2f%%", self.chartModel.priceChangeRatio*100];
            cell.detailTextLabel.text=[NSString stringWithFormat:@"%@%%",deal2(self.chartModel.priceChangeRatio*100.00,2)];
...
//    AudioServicesPlaySystemSound(1519);
    ...
    CGSize strSize = [self sizeWithFont:self.infoLabel.font infoStr:infoStr size:self.infoLabel.size];
...
- (CGSize)sizeWithFont:(UIFont *)font infoStr:(NSString *)infoStr size:(CGSize)size
{
    NSString *tmpText=infoStr;
    UIFont *tmpFont=font;
...
- (void)refreshCrossData
{
    ...
    cell5.detailTextLabel.text = [NSString stringWithFormat:@"%@%%",deal2(self.chartModel.priceChangeRatio*100.00,2)];
