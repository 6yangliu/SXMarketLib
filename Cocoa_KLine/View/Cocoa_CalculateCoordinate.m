...
void computeMAData(NSArray *items,int period)
{
    ...
    for (NSUInteger index = 0; index < items.count; index++) {
        ...
        [arrCls addObject:[NSString stringWithFormat:@"%0.10f",item.close]];
    }
...
/** BOLL计算 */
void computeBOLLData(NSArray *items)
{
    // 获取收盘价格
    NSMutableArray *arrCls = [[NSMutableArray alloc] init];
    for (NSUInteger index = 0; index < items.count; index++) {
    ...
    double *outRealMiddleBand = malloc(sizeof(double) * items.count);
    double *outRealLowerBand = malloc(sizeof(double) * items.count);
    
    // talib函数库，计算BOLL
    TA_RetCode ta_retCode = TA_BBANDS(0, (int) (items.count - 1), inCls, 20, 2,  2, TA_MAType_SMA, &outBegIdx, &outNBElement, outRealUpperBand, outRealMiddleBand, outRealLowerBand);
    
    if (TA_SUCCESS == ta_retCode) {
        for (NSInteger index = 0; index < arrCls.count; index++) {
            Cocoa_ChartModel *item = [items objectAtIndex:index];
            item.BOLL_UP = outRealUpperBand[arrCls.count-index-1];
            item.BOLL_MD = outRealMiddleBand[arrCls.count-index-1];
            item.BOLL_DN = outRealLowerBand[arrCls.count-index-1];
        }
    }
    freeAndSetNULL(inCls);
    freeAndSetNULL(outRealLowerBand);
    freeAndSetNULL(outRealMiddleBand);
...
void computeMACDData(NSArray *items)
{
    NSMutableArray *arrCls = [[NSMutableArray alloc] init];
    for (NSUInteger index = 0; index < items.count; index++) {
//        Cocoa_ChartModel *item = [items objectAtIndex:items.count-1-index];//数据貌似反了
        Cocoa_ChartModel *item = [items objectAtIndex:index];
        [arrCls addObject:[NSString stringWithFormat:@"%.*f",item.priceaccuracy,item.close]];
    }
    double *inCls = malloc(sizeof(double) * items.count);
    NSArrayToCArray(arrCls, inCls);
...
void computeKDJData(NSArray *items)
{
    ...
    for (NSUInteger index = 0; index < items.count; index++) {
        Cocoa_ChartModel *item = [items objectAtIndex:index];
        [arrHigval addObject:[NSString stringWithFormat:@"%.*f",item.priceaccuracy,item.high]];
    }
    ...
    for (NSUInteger index = 0; index < items.count; index++) {
        Cocoa_ChartModel *item = [items objectAtIndex:index];
        [arrLowval addObject:[NSString stringWithFormat:@"%.*f",item.priceaccuracy,item.low]];
    }
...
/** OBV计算 计算OBV非常简单。当今日收盘价高于昨日收盘价时，今日的成交量为“正值”。而当今日收盘价低于昨日收盘价时，则今日的成交量为“负值”。一连串时间的正负值成交量累积相加，即为OBV值。 */
void computeOBVData(NSArray *items)
{
    NSMutableArray *arrClose = [[NSMutableArray alloc] init];
    for (NSUInteger index = 0; index < items.count; index++) {
        Cocoa_ChartModel *item = [items objectAtIndex:index];
        [arrClose addObject:[NSString stringWithFormat:@"%.*f",item.priceaccuracy,item.close]];
    }
    double *inHigval = malloc(sizeof(double) * items.count);
    ...
    for (NSUInteger index = 0; index < items.count; index++) {
        Cocoa_ChartModel *item = [items objectAtIndex:index];
        [arrVolume addObject:[NSString stringWithFormat:@"%.*f",item.priceaccuracy,item.volume]];
    }
...
NSArray *CArrayToNSArray(const double inCArray[], int length, int outBegIdx, int outNBElement)
{
    ...
    for (NSInteger index = 0; index < length; index++)
    {
        ...
        {
            [outNSArray addObject:[NSString stringWithFormat:@"%.10f", inCArray[index - outBegIdx]]];
        } else{
            
            [outNSArray addObject:[NSString stringWithFormat:@"%.10f", 0.0f]];
        }
    }
    return outNSArray;
}
...
NSArray *MACDCArrayToNSArray(const double inCArray[], int length, int outBegIdx, int outNBElement, NSArray *items, MACDParameter parameter){
    ...
    for (NSInteger index = 0; index < length; index++) {
        if (index >= outBegIdx && index < outBegIdx + outNBElement) {
            [outNSArray addObject:[NSString stringWithFormat:@"%0.10f", inCArray[index - outBegIdx]]];
            ...
            switch (parameter) {
                case MACDParameterMACD:
                    [outNSArray addObject:[NSString stringWithFormat:@"%.10f",MACDValue]];
                    break;
                case MACDParameterDIFF:
                    [outNSArray addObject:[NSString stringWithFormat:@"%.10f",DIFFValue]];
                    break;
                case MACDParameterDEA:
                    [outNSArray addObject:[NSString stringWithFormat:@"%.10f",DEAValue]];
                    break;
                default:
                    [outNSArray addObject:[NSString stringWithFormat:@"%.10f", 0.0f]];
                    break;
