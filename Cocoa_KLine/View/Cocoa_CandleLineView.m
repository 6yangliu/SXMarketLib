...
/** BOLL线 */
@property (nonatomic,strong) CAShapeLayer *boll_md_LineLayer;
@property (nonatomic,strong) CAShapeLayer *boll_up_LineLayer;
@property (nonatomic,strong) CAShapeLayer *boll_dn_LineLayer;
...
- (void)initLayer
{
    ...
    [self.layer addSublayer:self.boll_md_LineLayer];
    [self.layer addSublayer:self.boll_up_LineLayer];
    [self.layer addSublayer:self.boll_dn_LineLayer];
...
- (void)clearChartView
{
    ...
    [self.boll_md_LineLayer removeFromSuperlayer];
    self.boll_md_LineLayer = nil;
    
    [self.boll_dn_LineLayer removeFromSuperlayer];
    self.boll_dn_LineLayer = nil;
    
    [self.boll_up_LineLayer removeFromSuperlayer];
    self.boll_up_LineLayer = nil;
...
#pragma mark - 清除K线,分时和指标之类
- (void)clearKline
{
    
    [self.highValueL removeFromSuperview];
    [self.lowValueL removeFromSuperview];
    self.highValueL=nil;
    self.lowValueL=nil;
...
- (void)drawChartView
{
    ...
    /** 绘制k线 分时线 */
    if(self.mainChartType==MainChartType_TimeLine){
        [self clearKline];
        [self drawTimeLineSublayers];
    }else{
        [self clearTimeLine];
        [self drawCandleSublayers];
    }
    /** 绘制k线 指标 */
    [self refreshMainTecnnical];
    // 回到数据
...
//            NSLog(@"%li",self.currentDisplayArray.count);
        }
...
//        minY = MIN(minY, MIN(entity.ma20, MIN(entity.ma10, entity.ma5)));
        ...
        // 8位精度
        if (maxY - minY < minAccuracyValue) {
            
            maxY += minAccuracyValue/2;
            minY -= minAccuracyValue/2;
        }
    }
    ...
    if (isnan(self.scaleValue)) {
        self.scaleValue=0;
    }
    if (isinf(self.scaleValue)) {
        self.scaleValue=0;
    }
    
    self.coordinateminValue = self.minValue - self.padding.bottom/self.scaleValue;
    if (isnan(self.coordinateminValue)) {
        self.coordinateminValue=0;
    }
    if (isinf(self.coordinateminValue)) {
        self.coordinateminValue=0;
    }
    
    self.coordinateMaxValue =  CGRectGetHeight(self.frame)/self.scaleValue + self.coordinateminValue;
    if (isnan(self.coordinateMaxValue)) {
        self.coordinateMaxValue=0;
    }
    if (isinf(self.coordinateMaxValue)) {
        self.coordinateMaxValue=0;
    }
}
#pragma mark-划K线
...
- (void)drawTimeLineSublayers
{
    ...
    for (NSInteger idx = 0; idx<self.currentDisplayArray.count; idx ++) {
        ...
        CGFloat xPostion = model.closePoint.x + self.candleWidth / 2;
        CGFloat yPostion = model.closePoint.y+self.padding.top;
        if(idx==0){
            [pathLine moveToPoint:CGPointMake(xPostion,yPostion)];
        }
        [pathLine addLineToPoint:CGPointMake(xPostion,yPostion)];
    }
    // 画分时线
    self.timeLineLayer.path =pathLine.CGPath;
    self.timeLineLayer.lineWidth = self.timeLineWidth;
    self.timeLineLayer.strokeColor = self.timeLineColor.CGColor;
...
- (void)drawMaSublayers
{
    ...
    self.ma1LineLayer.lineWidth = self.avgLineWidth;
...
- (void)drawBOLLSublayers
{
    ...
    for (NSInteger idx = 0; idx<self.currentDisplayArray.count; idx ++) {
        ...
        // 绘制上轨
        CGPoint BOLL_up_Point = CGPointMake(self.padding.left+self.leftPostion + ((self.candleWidth  + self.candleSpace) * idx) + self.candleWidth/2, ((self.maxValue - model.BOLL_UP) *self.scaleValue) + self.padding.top);
        
        if (idx == 0) {
            [pathLine1 moveToPoint:CGPointMake(BOLL_up_Point.x,BOLL_up_Point.y)];
        }else {
            if(model.localIndex==19){
                [pathLine1 moveToPoint:CGPointMake(BOLL_up_Point.x,BOLL_up_Point.y)];
            }else{
                [pathLine1 addLineToPoint:CGPointMake(BOLL_up_Point.x,BOLL_up_Point.y)];
            }
            
        }
        // 绘制中轨
        CGPoint BOLL_md_Point = CGPointMake(self.padding.left+self.leftPostion + ((self.candleWidth  + self.candleSpace) * idx) + self.candleWidth/2, ((self.maxValue - model.BOLL_MD) *self.scaleValue) + self.padding.top);
        
        if (idx == 0) {
            [pathLine2 moveToPoint:CGPointMake(BOLL_md_Point.x,BOLL_md_Point.y)];
        }else {
            if(model.localIndex==19){
                [pathLine2 moveToPoint:CGPointMake(BOLL_md_Point.x,BOLL_md_Point.y)];
            }else{
                [pathLine2 addLineToPoint:CGPointMake(BOLL_md_Point.x,BOLL_md_Point.y)];
            }
        }
        // 绘制下轨
        CGPoint BOLL_dn_Point = CGPointMake(self.padding.left+self.leftPostion + ((self.candleWidth  + self.candleSpace) * idx) + self.candleWidth/2, ((self.maxValue - model.BOLL_DN) *self.scaleValue) + self.padding.top);
        
        if (idx == 0) {
            [pathLine3 moveToPoint:CGPointMake(BOLL_dn_Point.x,BOLL_dn_Point.y)];
        }else {
            if(model.localIndex==19){
                [pathLine3 moveToPoint:CGPointMake(BOLL_dn_Point.x,BOLL_dn_Point.y)];
            }else{
                [pathLine3 addLineToPoint:CGPointMake(BOLL_dn_Point.x,BOLL_dn_Point.y)];
            }
            
        }
    }
    self.boll_up_LineLayer.path = pathLine1.CGPath;
    self.boll_up_LineLayer.lineWidth = self.bollLineWidth;
    self.boll_up_LineLayer.fillColor = [[UIColor clearColor] CGColor];
    self.boll_up_LineLayer.contentsScale = [UIScreen mainScreen].scale;
    
    self.boll_md_LineLayer.path = pathLine2.CGPath;
    self.boll_md_LineLayer.lineWidth = self.bollLineWidth;
    self.boll_md_LineLayer.fillColor = [[UIColor clearColor] CGColor];
    self.boll_md_LineLayer.contentsScale = [UIScreen mainScreen].scale;
    
    self.boll_dn_LineLayer.path = pathLine3.CGPath;
    self.boll_dn_LineLayer.lineWidth = self.bollLineWidth;
    self.boll_dn_LineLayer.fillColor = [[UIColor clearColor] CGColor];
    self.boll_dn_LineLayer.contentsScale = [UIScreen mainScreen].scale;
}
...
- (void)refreshMainTecnnical{
    //先清除指标
    [self clearMainTecnnical];
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
 
    if (self.mainTecnnicalType==MainTecnnicalType_MA){ /** 绘制均线 */
        [self drawMaSublayers];
    }
    if (self.mainTecnnicalType==MainTecnnicalType_BOLL){ /** 绘制均线 */
        [self drawBOLLSublayers];
    }
    [CATransaction commit];
}
#pragma mark - 绘制贝塞尔曲线
...
{
    if (!_timeLineWidth) _timeLineWidth = 1.0;
...
- (CGFloat)bollLineWidth
{
    if (!_bollLineWidth) _bollLineWidth = 1.0;
    return _bollLineWidth;
}
...
{
    if (!_timeLineColor) _timeLineColor = COLOR_timeLine;
    return _timeLineColor;
}

- (CAShapeLayer *)boll_md_LineLayer
{
    if (!_boll_md_LineLayer) {
        
        _boll_md_LineLayer = [CAShapeLayer layer];
        _boll_md_LineLayer.lineWidth = 1.0;
        _boll_md_LineLayer.lineCap = kCALineCapRound;
        _boll_md_LineLayer.lineJoin = kCALineJoinRound;
        _boll_md_LineLayer.strokeColor=COLOR_BOLL_MD.CGColor;
    }
    
    return _boll_md_LineLayer;
}
- (CAShapeLayer *)boll_up_LineLayer
{
    if (!_boll_up_LineLayer) {
        
        _boll_up_LineLayer = [CAShapeLayer layer];
        _boll_up_LineLayer.lineWidth = 1.0;
        _boll_up_LineLayer.lineCap = kCALineCapRound;
        _boll_up_LineLayer.lineJoin = kCALineJoinRound;
        _boll_up_LineLayer.strokeColor=COLOR_BOLL_UP.CGColor;
    }
    
    return _boll_up_LineLayer;
}
- (CAShapeLayer *)boll_dn_LineLayer
{
    if (!_boll_dn_LineLayer) {
        
        _boll_dn_LineLayer = [CAShapeLayer layer];
        _boll_dn_LineLayer.lineWidth = 1.0;
        _boll_dn_LineLayer.lineCap = kCALineCapRound;
        _boll_dn_LineLayer.lineJoin = kCALineJoinRound;
        _boll_dn_LineLayer.strokeColor=COLOR_BOLL_DN.CGColor;
    }
    
    return _boll_dn_LineLayer;
}
